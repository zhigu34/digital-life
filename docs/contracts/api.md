...
Show: `{id,title,media_type,status,progress,total,score,notes,update_weekday}`. title 1–160, media_type anime|tv|movie default tv, status planned|watching|completed|paused default planned, progress integer ≥0 default 0, total nullable integer ≥1 default null, progress≤total if known. score nullable integer 1–10. notes max4000 default '', update_weekday null or 0–6 Monday–Sunday. `POST /shows/<id>/advance` increments progress, sets watching or completed at total, at total returns unchanged. PATCH accepts all except id.
