...
专项开销（装修、旅行、考证）需要单独归集，但又不能把账户余额切碎。结论：**账本是贴在流水与账单上的标签，不是第二套账户**。

- 层级：`book_id` 挂在 `ledger_entries` 与 `expenses` 上；账户、分类、商户仍全局共享，余额仍在账户维度派生，跨账本合计保持连续 —— 切账本永远不改变任何余额。
- 迁移 `0010` 新建 `ledger_books` 并给两列加可空 `book_id`（SQLite 不能 `ADD COLUMN` 带外键，故不写 REFERENCES），同时为每个已有账号补建「日常」并把历史行归入，避免升级后被读成数据丢失。
- 默认账本：`ensure_default_book(db, user_id)` 取 `sort_order` 最小者，没有则播种「日常」；`GET /books` 会调用它，手工记账未指定 `book_id` 时落到它。前端只对真正为 null 的行显示「未归类」。
- 口径：`/api/stats?book_id=` 把账单应付与流水两路口径同时收窄；维护费用与追剧汇总不属账本维度。列表只有 `listEntries` 支持服务端 `book_id` 过滤 —— 所有账本的流水本来就一次取回，切换才能即时。
- 删除被流水或账单引用的账本返回 409，改用归档从选择器退出（流水保留）。
- 导入：`ledger_books` 全量重建并按 id 重映射；**旧版导出（无该键）含账单或流水时要在同一事务内补建默认账本**，不能用会 `commit()` 的播种助手 —— 否则被拒的导入会先把调用方账本清空。
