...
- 任务：一次性待办（状态、优先级、截止日期）与长期任务同页管理。长期任务是一个分组，组内可以放多个打卡项，每项按每天 / 本周内完成 / 本月内完成计量；周期内完成一次即达标，卡片上以格子显示哪些周期已达标、哪些漏做、哪个还在进行中，可补记任意过去日期并查看某个周期的明细；今日总览把待办与待完成的长期任务合并成一个列表，可一键打卡。长期任务卡片默认折叠，只显示分组名称、本期进度与每个打卡项的实时状态摘要，展开后才是周期格子、耗时统计（已坚持天数 / 最近打卡 / 累计次数 / 周期达标）和最近 12 条执行日志；折叠偏好按账号记在浏览器里，工具条上的「全部折叠 / 全部展开」可一次处理当前列表。
