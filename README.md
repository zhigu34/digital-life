...
脚本根据源文件内容判断受影响服务：只改前端就只重建前端。构建前会检查并按需拉取基础镜像、校验架构匹配、检测 Web 端口冲突和磁盘空间；构建完成后才停止需要更新的后端。数据库结构有待执行迁移时，先停服并制作一致性备份再迁移；结构已是最新则跳过停服备份直接更新（成功部署后自动只保留最近 `DEPLOY_KEEP_PREDEPLOY_BACKUPS`（默认 10）份 `pre-deploy-*` 自动备份，手动备份不受影响）。Web 和后端健康检查都通过后才记录成功版本。
