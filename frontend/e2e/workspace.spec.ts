...
test('check-ins track daily streaks and stay private', async ({ page }, testInfo) => {
  ...
  await navigate(page, '今日总览')
  await expect(page.getByText('今天还没打卡', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '打卡 健身1小时', exact: true }).click()
  await expect(page.getByText('已打卡，继续保持', { exact: true })).toBeVisible()
  await expect(page.getByText('今天还没打卡', { exact: true })).toHaveCount(0)
  await navigate(page, '打卡')
  const habitCard = page.locator('article').filter({ hasText: '健身1小时' })
  await expect(habitCard).toContainText('连续 1')
  // Undo today, then redo directly on the card.
  await page.getByRole('button', { name: '撤销今天打卡 健身1小时', exact: true }).click()
  await expect(page.getByText('已撤销今天的打卡', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '打卡 健身1小时', exact: true }).click()
  await expect(habitCard).toContainText('连续 1')
  // Makeup yesterday through the history manager.
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai' }).format(new Date())
  const yesterday = new Date(Date.parse(`${today}T00:00:00Z`) - 86400000).toISOString().slice(0, 10)
  await page.getByRole('button', { name: '管理打卡记录 健身1小时', exact: true }).click()
  dialog = page.getByRole('dialog')
  await dialog.getByLabel('日期', { exact: true }).fill(yesterday)
  ...
  await navigate(page, '打卡')
  await expect(habitCard).toContainText('连续 2')
  await page.screenshot({ path: testInfo.outputPath('checkins.png'), fullPage: true })
  expect(await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth + 1)).toBe(false)
  await navigate(page, '个人设置')
  await page.getByRole('button', { name: '退出登录', exact: true }).filter({ visible: true }).first().click()
  await login(page, bob)
