...
test('long-term tasks check in by period and stay private', async ({ page }, testInfo) => {
  const alice = await account(), bob = await account()
  await login(page, alice)
  await navigate(page, '任务')
  await page.getByRole('button', { name: '添加长期任务', exact: true }).first().click()
  let dialog = page.getByRole('dialog')
  await dialog.getByLabel('分组名称', { exact: true }).fill('健身计划')
  await dialog.getByLabel('打卡项', { exact: true }).fill('跑步 30 分钟')
  await dialog.getByRole('button', { name: '保存', exact: true }).click()
  await expect(dialog).toHaveCount(0)
  const card = page.locator('article').filter({ hasText: '健身计划' })
  await expect(card).toContainText('跑步 30 分钟')
  await expect(card).toContainText('每天')
  await expect(card).toContainText('今天还没打卡')

  // One tap from the today overview: the shell hands the check to the task page.
  await navigate(page, '今日总览')
  await expect(page.getByText('今天的任务', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '打卡 跑步 30 分钟', exact: true }).click()
  await expect(page.getByText('已打卡', { exact: false }).first()).toBeVisible()
  await expect(card).toContainText('连续 1 天')
  await expect(card).toContainText('今天已完成')

  // Undo, then check again directly on the card.
  await page.getByRole('button', { name: '撤销打卡 跑步 30 分钟', exact: true }).click()
  await expect(page.getByText('已撤销今天的打卡', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '打卡 跑步 30 分钟', exact: true }).click()
  await expect(card).toContainText('连续 1 天')

  // Make up yesterday inside the running period.
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai' }).format(new Date())
  const yesterday = new Date(Date.parse(`${today}T00:00:00Z`) - 86400000).toISOString().slice(0, 10)
  await page.getByRole('button', { name: '跑步 30 分钟 的历史明细', exact: true }).click()
  dialog = page.getByRole('dialog')
  await dialog.getByLabel('补记日期', { exact: true }).fill(yesterday)
  await dialog.getByLabel('当天备注', { exact: false }).fill('补昨天的卡')
  await dialog.getByRole('button', { name: '记一笔', exact: true }).click()
  await expect(dialog).toContainText('补昨天的卡')
  await dialog.getByRole('button', { name: '完成', exact: true }).click()
  await expect(card).toContainText('连续 2 天')
  await expect(card).toContainText('累计 2 次')

  // A weekly item is satisfied by any completion inside the running week.
  await page.getByRole('button', { name: '为 健身计划 添加打卡项', exact: true }).click()
  dialog = page.getByRole('dialog')
  await dialog.getByLabel('打卡项', { exact: true }).fill('力量训练')
  await dialog.getByLabel('打卡周期').selectOption('week')
  await dialog.getByRole('button', { name: '保存', exact: true }).click()
  await expect(dialog).toHaveCount(0)
  const weekRow = card.locator('.item-row').filter({ hasText: '力量训练' })
  await expect(weekRow).toContainText('本周还没完成')
  await weekRow.locator('.checkin-hit').click()
  await expect(weekRow).toContainText('本周已完成 1 次')

  await page.reload()
  await navigate(page, '任务')
  await expect(card).toContainText('连续 2 天')
  await expect(card).toContainText('本周已完成 1 次')
