...
const props = defineProps<{ stats: Stats | null; createRequest?: number }>();
const emit = defineEmits<{ sync: [shows: Show[]]; error: [error: unknown]; notice: [message: string] }>();
const { shows, loading, busy, error, load, create, update, remove, advance } = useShows();
const editing = ref<Show | null | undefined>(undefined), formError = ref(""), search = ref("");
...
async function reload(){try{await load();emit("sync",[...shows.value]);}catch(e){emit("error",e);}}
function open(item?:Show){formError.value="";editing.value=item??null;}
function close(){editing.value=undefined;formError.value="";}
function sync(){emit("sync",[...shows.value]);}
async function save(data:Record<string,unknown>){formError.value="";try{if(editing.value)await update(editing.value.id,data);else await create(data);const wasEdit=!!editing.value;close();sync();emit("notice",wasEdit?"记录已更新":"已添入你的日常");}catch(e){formError.value=e instanceof Error?e.message:"保存失败";emit("error",e);}}
async function removeItem(item:Show){if(!window.confirm(`确定删除“${item.title}”？删除后无法恢复。`))return;try{await remove(item.id);close();sync();emit("notice","记录已删除");}catch(e){emit("error",e);}}
async function advanceItem(id:number){try{await advance(id);sync();emit("notice","又看完一集，进度已更新");}catch(e){emit("error",e);}}
watch(()=>props.createRequest,(next,previous)=>{if(next!==undefined&&next!==previous)open();});
onMounted(()=>{void reload();if(props.createRequest)open();});
</script>

<template>
  <section class="page collection-page shows-page">
    <div v-if="error" class="global-error" role="alert"><span>{{error}}</span><button class="text-button" @click="reload">重新加载</button></div><div v-if="loading" class="loading-line" role="status" aria-label="正在同步追剧记录"></div>
    ...
    <EmptyState v-if="!loading&&!shows.length" icon="shows" title="下一段好故事，等你开启" description="添加一部想看的电影、剧集或动漫，慢慢享受。" action="添加作品" @action="open()" /><EmptyState v-else-if="!loading&&!filtered.length" icon="search" title="没有找到对应记录" description="试试其他关键词，或切换状态筛选。" />
    ...
    <p v-if="shows.length" class="page-footnote">一点一滴，都是生活的痕迹。</p><ShowForm v-if="editing!==undefined" :item="editing||undefined" :busy="busy" :error="formError" @close="close" @save="save" @remove="removeItem" />
  </section>
</template>
