...
async function login(data:{username:string;password:string}){busy.value=true;loginError.value="";try{const session=await api<{user:User;csrf_token:string}>("/auth/login","POST",data);user.value=session.user;setCsrf(session.csrf_token);await load();}catch(e){loginError.value=e instanceof Error?e.message:"登录失败";}finally{busy.value=false;}}
