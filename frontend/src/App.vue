...
function navigate(next:Page,ledgerTab?:LedgerTab){moreOpen.value=false;page.value=next;editing.value=null;showCreateRequest.value=0;expenseCreateRequest.value=0;ledgerStartTab.value=next==="expenses"?ledgerTab??null:null;window.scrollTo({top:0,behavior:"smooth"});}
function open(collection:GenericCollection,item?:RecordItem){formError.value="";editing.value={collection,item};}
function createFromToday(collection:Collection){if(collection==="shows"){navigate("shows");showCreateRequest.value++;return;}if(collection==="expenses"){navigate("expenses");expenseCreateRequest.value++;return;}open(collection);}
